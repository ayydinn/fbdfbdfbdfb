...
module.exports = async (client, member) => {
    ...
    if (welcome[member.guild.id]) {
        if (welcome[member.guild.id].func === true) {
            ...
            if (member.guild.channels.cache.has(channel)) {
                ...
                .setColor('username-box', '#2BAC95')
                //.setColor('discriminator-box', '#2BAC95')
                .setColor('message-box', '#2BAC95')
                .setColor('title', '#ffffff')
                .setColor('avatar', '#2BAC95')
                //.setBackground('https://unsplash.com/photos/dtmz4CDRXqw')
                //.setBackground("https://png.pngtree.com/thumb_back/fw800/back_pic/00/11/26/5356376b0b393f6.jpg")
                //.toAttachment();
    ...
    if (client.muteRisks.has(member.id)) {
        try {
                ...
                member.roles.add(muteRole, 'Member tried to leave and rejoin to bypass a mute');
        } catch (error) {
