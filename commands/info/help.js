...
module.exports.run = async (client, message, args) => {
        ...
        if (client.commands.has(args[0]) || client.commands.has(client.commands.get(client.aliases.get(args[0]).config.name))) {
            ...
            return message.channel.send(
                new MessageEmbed()
                    ...
                    .setTitle(`${pu.config.name} command`)
    ...
    if (!args[0]) {
        ...
        mainEmbed.setTitle(`${client.botName} commands`)
        mainEmbed.setURL(`https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot`)
        mainEmbed.setDescription(`\n Type \`help [command]\` for help with a command \n\n \`1\` **main menu** - *Shows commands groups*\n${groupsJoin.join('\n')}`, true)
        ...
        //const nahlohEmbed = createEmbed('nahloh')
        //const imageEmbed = createEmbed('image')

        const pages = [mainEmbed, moderationEmbed, utilityEmbed, infoEmbed, configEmbed, managementEmbed]
